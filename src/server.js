...
export function createMcp(engine) {
  const server = new McpServer({name:'kebiao',version:'0.1.0'}, {instructions:
    ...
    '修改可用精确 ID 或唯一完整标题，歧义会返回候选项，勿猜测。修改课程只影响课程，单次调课用 exception。' +
    ...
    'scheduleStatus: scheduled=已录完整时间、partial=缺部分课次、unknown=未录入/未知、tbd=来源确认待定；空 rules 默认 unknown，勿说成学校没有排课。expectedTiming 可存已知开课周/日期/持续周数，不得编造星期节次。unscheduled 查本范围资料不全的课程，state=all 包括未选。' +
    'agenda/conflicts/free/unscheduled 及模拟返回 coverage，complete=false 时必须说明跳过/部分分析的原因，conflicts=[] 不代表没有冲突；coverage 不受展示筛选影响。模拟返回 simulation 新增冲突及目标是否生效；plan.tentative=true 表示只避开已知时间，无法保证不撞待定课程。' +
  ...
  const definitions=[
    ['schedule_query','组合查询：一次获取课表、完整课程资料、任务、冲突、空闲、学期配置、学分学时汇总 summary、时间资料不全的 unscheduled。scheduleStatus/category 可筛选，catalog 显式返回时间状态。courseStatus/termId 可筛选；summary 排除 category=activity，返回 knownTotal、missing、knownCount 和 available，按学期/状态分组。默认未来7天、仅启用；查未选课程设 state=all。simulateEnable 可按 ID/唯一标题临时启用做冲突分析，同时返回 simulation 的新增冲突与每个目标是否生效。coverage 披露缺失时间，complete=false 不可宣称无冲突；覆盖情况不受显示筛选影响。free/conflicts 始终基于全部启用占用，不受 search/state/kinds/ids 过滤；config 返回 timetables、timetableSwitches 和查询起点在学期当地日期的 activeTimetable（学期外为 null）；用 termId 筛选。结果逐视图分页。',querySchema,'query',true],
    ...
    ['schedule_import','批量导入结构化官方数据（不下载/解析网页或图片）。每条须有 source.namespace/key；公示课表应含未选课堂的 rules，各课堂用独立 key；未选不是无时间。缺数据用 scheduleStatus=unknown，确认未定用 tbd，部分录入用 partial；可填 expectedTiming 和 scheduleSource。不能自动抓取缺失数据。完整替换时需携带保留的 category/时间信息。本次 namespace 统一来源，key 稳定去重。新课程默认禁用，明确 enabled=true 表示已选；重新导入默认保留个人 enabled 和已记录的 status，分别用 preserveEnabled/preserveStatus=false 覆盖。显式选课状态使用 status，enabled 仅表示启禁用。条目为完整替换；未在本次出现的旧条目保留。可同批导入学期和课程，引用使用固定 ID。',importSchema,'import',false],
